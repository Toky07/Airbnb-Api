...
export function getInvoiceBrand(
  env: EnvSource = process.env,
): InvoiceBrandConfig {
  return {
    name:
      readEnv(env, 'INVOICE_BRAND_NAME') ??
      readEnv(env, 'BRAND_NAME') ??
      'StayBook',
    color: readEnv(env, 'INVOICE_BRAND_COLOR') ?? '#FF385C',
    supportEmail:
      readEnv(env, 'SUPPORT_EMAIL') ??
      readEnv(env, 'INVOICE_SUPPORT_EMAIL') ??
      'support@staybook.app',
...
export function getInvoiceIssuer(
  ...
  return {
    name:
      readEnv(env, 'INVOICE_ISSUER_NAME') ??
      readEnv(env, 'BRAND_NAME') ??
      'StayBook',
