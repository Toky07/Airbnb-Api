...
export type SendAccountInvitationOptions = {
  userId: number;
  sourceModule?: string;
};

@Injectable()
export class SendAccountInvitationUseCase {
  constructor(
  ...
  async execute(options: SendAccountInvitationOptions): Promise<void> {
    ...
    await this.mailService.sendSimple({
      ...
      sourceModule: options.sourceModule ?? 'account-invitation',
    });
  }
}
