...
import { UpdateAmenityUseCase } from './update-amenity.usecase';
...
describe('UpdateAmenityUseCase', () => {
  it('updates an amenity', async () => {
    ...
    const useCase = new UpdateAmenityUseCase(repository);
    const result = await useCase.execute(1, { name: 'WiFi Pro' });
  ...
  it('throws when amenity is missing', async () => {
    ...
    await expect(useCase.execute(99, { name: 'TV' })).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
});
