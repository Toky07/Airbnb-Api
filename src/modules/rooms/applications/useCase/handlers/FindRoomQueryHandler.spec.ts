...
describe('FindRoomQueryHandler', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        mockExecute.mockImplementation(async (query: { constructor: { name: string } }) => {
            if (query.constructor.name === 'ListRoomAmenitiesQuery') {
                return [{ id: 1, name: 'WiFi', icon: 'wifi', scope: 'room', isActive: true }];
            }
            if (query.constructor.name === 'ListPropertyAmenitiesQuery') {
                return [{ id: 2, name: 'Piscine', icon: 'water-ladder', scope: 'property', isActive: true }];
            }
            return [];
        });
    });
    ...
    it('should throw an error if the room is not found', async () => {
        ...
        await expect(handler.execute(new FindRoomQuery({ id: 2 }))).rejects.toThrow('Room not found');
    });
});
